...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"crx": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + crx.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + crx.blockNumber + " (" + new Date(1000 * crx.getBlock(crx.blockNumber).timestamp) + ")\n";
		try {
